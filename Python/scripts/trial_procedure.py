"""
Script for running trial procedure on external machine.
Tensorflow version ?
"""
...
def mat_from_idx(idx, y=None):
    if y is None:
        ...
        return tf.stack((
            tf.constant(1, shape=len(idx)),
            df['length'].iloc[idx],
            df['length'].iloc[idx]*(df['sex'].iloc[idx] == 'male'), y
        ), axis=1)
...
def build_model():
    ...
    _model.compile(tf.keras.optimizers.Adam(0.00046625), tf.keras.losses.MeanSquaredError())
...
def build_linear_model(num_predictors):
    ...
    _linear_model = tf.keras.models.Model(inputs=inputs, outputs=outputs)

    _linear_model.compile(tf.keras.optimizers.Adam(1e-3), tf.keras.losses.MeanSquaredError())
...
# Create dataframes for storing summary results and individual results
...
for i in range(len(strata_idxs)):
    ...
    # Predict age of test data using deep learning
    ...
    y2 = mat_from_idx(test_idx) @ w
    summary['loss2'].iloc[i] = tf.keras.losses.mean_squared_error(y2, df['age'].iloc[test_idx])
    ...
    linear_model.fit(
        ...
        epochs=max_epochs,
        validation_data=(
            mat_from_idx(valid_idx, y=model.predict(set_from_idx(valid_idx)).flatten()),
            df['age'].iloc[valid_idx]
        ),
        callbacks=callbacks
    )
