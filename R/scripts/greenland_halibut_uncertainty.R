...
results = read.csv(r'(C:\Users\iverm\OneDrive - UiT Office 365\UiT\Deep learning applied to fish otolith images\Blåkveiteotolitter\Validation Procedure\results.csv)')
summary = read.csv(r'(C:\Users\iverm\OneDrive - UiT Office 365\UiT\Deep learning applied to fish otolith images\Blåkveiteotolitter\Validation Procedure\summary.csv)')


draw_differences = function(y, title){
  #' Displays differences between labeled distribution and predicted distribution
  #'
...
for(i in 5:7){
  ...
  for(j in 0:2){
    ...
    print(paste0(j, '-off percentage females: ', sum(abs(results_female$age - round(results_female[, i])) <= 2)*100 / dim(results_female)[1]))
  }
  print('')
}
...
df.temp = results
df.temp$age[which(df.temp$sex == 'male')] = df.temp$age[which(df.temp$sex == 'male')] + 0.3

# Scatter plot predicted age vs labeled age
ggplot(df.temp, aes(x = age, y = round(y3))) + 
...
df.temp = data.frame(
  x = c(1:26, 1:26, 1:26), 
  y = y_vals, 
  f = c(rep('deep_learning', 26), rep('length', 26), rep('combined', 26))
  )

# Plot MSE contributions against age for all three models
ggplot(df.temp, aes(x = x, y = y, fill = factor(f, levels = c('length', 'deep_learning', 'combined')))) + 
